...
def load_model(args):
    ...
    depth_encoder = Damba2.DepthEncoder(args.num_layers, False)
    depth_decoder = Damba2.DepthDecoder(depth_encoder.num_ch_enc, scales=range(4))

    depth_encoder.load_state_dict({k: v for k, v in model["encoder"].items() if k in depth_encoder.state_dict()})
    depth_decoder.load_state_dict({k: v for k, v in model["depth"].items() if k in depth_decoder.state_dict()},strict=False)
